...
def format_test_results(results: List[CodeTestResult]) -> str:
    ...
    if failed == 0:
        ...
        for r in results:
            example_name = rf"{'Non-Compliant' if r.example_type == 'non_compliant' else 'Compliant'} \#{r.example_number}"
            if r.passed:
        ...
        if failures:
            ...
            for r in failures:
                example_name = rf"{'Non-Compliant' if r.example_type == 'non_compliant' else 'Compliant'} Example \#{r.example_number}"
