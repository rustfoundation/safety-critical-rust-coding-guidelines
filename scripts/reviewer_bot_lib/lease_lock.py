...
def lock_is_currently_valid(bot, lock_meta: dict, now: datetime | None = None) -> bool:
...
def build_lock_metadata(bot, lock_token: str, lock_owner_run_id: str, lock_owner_workflow: str, lock_owner_job: str) -> dict:
...
def clear_lock_metadata(bot) -> dict:
...
def parse_lock_metadata_from_lock_commit_message(bot, message: str) -> dict:
...
def ensure_lock_ref_exists(bot) -> str:
...
def get_lock_ref_snapshot(bot) -> tuple[str, str, dict]:
...
def create_lock_commit(bot, parent_sha: str, tree_sha: str, lock_meta: dict):
...
def cas_update_lock_ref(bot, new_sha: str):
    return bot.github_api_request(
        "PATCH",
...
def ensure_state_issue_lease_lock_fresh(bot) -> bool:
    context = bot.ACTIVE_LEASE_CONTEXT
    if context is None:
        return False
...
def renew_state_issue_lease_lock(bot, context: LeaseContext) -> bool:
...
def acquire_state_issue_lease_lock(bot) -> LeaseContext:
...
def release_state_issue_lease_lock(bot) -> bool:
    context = bot.ACTIVE_LEASE_CONTEXT
    if context is None:
        return True
    released = False
